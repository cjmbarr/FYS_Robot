...
thresholdVolatage = 2.80;
numberLights = 0;
while numberLights ~= 2
ldrDat = scanLDR(myKAR,ldrPin,frontScanServo,thresholdVolatage);
PlotUSPolar(ldrDat)
voltageData = ldrDat(:,2);
angleLDRData = ldrDat(:,1);
lightData = voltageData(voltageData ~= 0);
angleLightData = angleLDRData(voltageData ~= 0);
     if (length(lightData) < 28) & (length(lightData) > 0)
         numberLights = 1;
     elseif length(lightData) >= 28
         numberLights = 2;
     else
         numberLights = 0;
     end
   ...
   Spin(0,myKAR,in1,in2,in3,in4,enA,enB,0,powerOutput);
   Move(0,myKAR,in1,in2,in3,in4,enA,enB,0,powerOutput);
   elseif numberLights == 1
   oneLightData = voltageData(voltageData ~= 0);
   oneAngleLightData = angleLDRData(voltageData ~= 0);
   oneLightAngle = mean(oneAngleLightData);
   Spin(oneLightAngle,myKAR,in1,in2,in3,in4,enA,enB,oneLightAngle,powerOutput)
   Move(5,myKAR,in1,in2,in3,in4,enA,enB,0,powerOutput);
   else
   Spin(90,myKAR,in1,in2,in3,in4,enA,enB,0,powerOutput);
   Move(-6,myKAR,in1,in2,in3,in4,enA,enB,-1,powerOutput);

   end
end


lightAngle = mean(angleLightData);
Spin(lightAngle,myKAR,in1,in2,in3,in4,enA,enB,lightAngle,powerOutput);
gateDistance = (readDistance(frontUSsensor));
Move(gateDistance*100 + 35,myKAR,in1,in2,in3,in4,enA,enB,0,powerOutput);
Spin(0,myKAR,in1,in2,in3,in4,enA,enB,0,powerOutput);
Move(0,myKAR,in1,in2,in3,in4,enA,enB,0,powerOutput);
...
function [ldrDat] = scanLDR(myKAR, ldrPin, frontScanServo,thresholdVoltage)
 writePosition(frontScanServo, 0.5); % Set servo to center
 pause(0.5)
 fprintf('Get ready for some scanning!\n');
 
    ldrDat = zeros(181,2); % Creates the data array
    n = linspace(1,0,181);
    ldrDat(:,1) = [-90:90]';
    for  i = 1:181 % starts a for loop to update the array for every position
        writePosition(frontScanServo,n(i));
        pause(0.0015) % Give time for the servo to read and move again
        firstLDR_read = readVoltage(myKAR,ldrPin);
        while firstLDR_read == Inf
            firstLDR_read = readVoltage(myKAR,ldrPin);
        end
        pause(0.0015)
        secondLDR_read = readVoltage(myKAR,ldrPin);
        while secondLDR_read == Inf
            secondLDR_read = readVoltage(myKAR,ldrPin);
        end
        pause(0.0015)
        thirdLDR_read = readVoltage(myKAR,ldrPin);
        while thirdLDR_read == Inf
            thirdLDR_read = readVoltage(myKAR,ldrPin);
        end
        ldrDat(i,2) = (firstLDR_read+secondLDR_read+thirdLDR_read)/3;
        if ldrDat(i,2) > thresholdVoltage
            ldrDat(i,2) = 0; 
        end
        pause(0.0015)
    end
