...
powerOutput = 11.8915 / batteryLevel;
...
while isempty(minAngles)
   ...
   if isempty(minAngles)
       SpinScan(80,myKAR,in1,in2,in3,in4,enA,enB,powerOutput);
       Move(-18.5,myKAR,in1,in2,in3,in4,enA,enB,-1,powerOutput);
   else
       SpinScan(0,myKAR,in1,in2,in3,in4,enA,enB,powerOutput);
...
% 
% parallaxOffset = 7.1; 
% sensorRangeOffset = 1.8;
% robotRangeOffset = 8.9; 
% 
% 
% 
% [thetaR,distanceR] = ParallaxConversion(thetaS,distanceS,sensorRangeOffset,parallaxOffset,robotRangeOffset);
% 
% 

% 
% Spin(spin,myKAR,in1,in2,in3,in4,enA,enB,thetaR);
% Move(move,myKAR,in1,in2,in3,in4,enA,enB);
...
function SpinScan(spin,myKAR,in1,in2,in3,in4,enA,enB,powerOutput)
    speedMotors = 2;
    spin2 = abs((spin/90)*1.50)
    % Assign Left and Right Rotation Direction
    writeDigitalPin(myKAR,in1,0); writeDigitalPin(myKAR,in2,0); % A
    writeDigitalPin(myKAR,in4,0); writeDigitalPin(myKAR,in3,1); % B
    % Assign Speed to both motors
    writePWMVoltage(myKAR,enA,speedMotors*powerOutput); % Left
    writePWMVoltage(myKAR,enB,speedMotors*powerOutput); % Right
...
function Move(move,myKAR,in1,in2,in3,in4,enA,enB,distanceR,powerOutput)
    ...
    % Assign Speed to both motors
    writePWMVoltage(myKAR,enA,speedMotors*powerOutput); % Left
    writePWMVoltage(myKAR,enB,speedMotors*0.82*powerOutput); % Right
